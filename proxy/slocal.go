...
func CreateClient(conf Config) {
	log.SetOutput(os.Stdout)
	var cmdConfig ss.Config

	cmdConfig.LocalAddress = conf.LocalAddr
	cmdConfig.LocalPort = conf.LocalPort
	cmdConfig.Password = conf.PassWord
	cmdConfig.ServerPort = conf.RemotePort
	cmdConfig.Method = conf.Method
	cmdConfig.Timeout = 300
	cmdConfig.Server = conf.Server
	config := &ss.Config{}
	ss.SetDebug(debug)
